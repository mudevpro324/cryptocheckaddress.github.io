(function() {
    ...
    DOM.addressMatchTarget = $("#address-match-target");
    ...
    function getMnemonicObjectForPhrase(phrase) {
        ...
        return mnemonics[language];
    }

    function buildAddressSetForPhrase(phrase, passphrase) {
        var normalized = phrase.trim();
        if (!normalized) {
            return [];
        }

        var phraseMnemonic = getMnemonicObjectForPhrase(normalized);
        if (!phraseMnemonic.check(normalized)) {
            return [];
        }

        var previousMnemonic = mnemonic;
        var previousSeed = seed;
        var previousRootKey = bip32RootKey;
        var previousExtendedKey = bip32ExtendedKey;
        var checksumPass = passphrase || "";

        mnemonic = phraseMnemonic;
        calcBip32RootKeyFromSeed(normalized, checksumPass);

        var addressSet = [];
        var derivations = [
            { label: 'BTC Legacy', path: "m/44'/0'/0'/0", type: 'btc' },
            { label: 'BTC Segwit P2SH', path: "m/49'/0'/0'/0", type: 'btc' },
            { label: 'BTC Bech32', path: "m/84'/0'/0'/0", type: 'btc' },
            { label: 'ETH Ledger/Trezor', path: "m/44'/60'/0'/0", type: 'eth' },
            { label: 'ETH Legacy', path: "m/44'/60'/0'", type: 'eth' }
        ];

        for (var i = 0; i < derivations.length; i++) {
            var derivation = derivations[i];
            var address = DerivePublicAddress(derivation.path);
            addressSet.push({
        ...
        seed = previousSeed;
        bip32RootKey = previousRootKey;
        bip32ExtendedKey = previousExtendedKey;
        return addressSet;
    }

    function checkAddressMatches() {
        var targetAddress = normalizeAddress(DOM.addressMatchTarget.val());
        var candidateText = DOM.addressMatchCandidates.val();
        var candidatePhrases = candidateText.split(/\n+/);
        var results = [];

        if (!targetAddress) {
            DOM.addressMatchResults.html('<div class="alert alert-warning">Please enter a target Bitcoin or Ethereum address.</div>');
            return;
        }

        if (!candidateText || candidateText.trim().length === 0) {
            DOM.addressMatchResults.html('<div class="alert alert-warning">Please provide at least one candidate mnemonic.</div>');
            return;
        }

        for (var i = 0; i < candidatePhrases.length; i++) {
            var phrase = candidatePhrases[i].trim();
            if (!phrase) {
                continue;
            }

            var wordCount = getWordCount(phrase);
            if (wordCount !== 12 && wordCount !== 18 && wordCount !== 24) {
                results.push({
                    phrase: phrase,
                    match: false,
                    reason: 'Invalid length: ' + wordCount + ' words. Only 12, 18, or 24 word mnemonics are supported.'
                });
                continue;
            }

            var phraseMnemonic = getMnemonicObjectForPhrase(phrase);
            if (!phraseMnemonic.check(phrase)) {
                results.push({
                    phrase: phrase,
                    match: false,
                    reason: 'Not a valid BIP39 mnemonic.'
                });
                continue;
            }
            ...
            if (found.length > 0) {
                ...
                results.push({
                    phrase: phrase,
                    match: false,
                    reason: 'No matching address found for the provided target.'
                });
            }
        }

        if (results.length === 0) {
            DOM.addressMatchResults.html('<div class="alert alert-info">No candidate mnemonics were provided.</div>');
            return;
        }

        var html = '<div class="alert alert-info">Checked ' + results.length + ' candidate seed phrase(s).</div>';
        html += '<ul class="list-group">';
        for (var k = 0; k < results.length; k++) {
            var result = results[k];
            if (result.match) {
                html += '<li class="list-group-item list-group-item-success"><strong>' + result.phrase + '</strong><br>Matches target address in: ' + result.found.join(', ') + '</li>';
            }
            else {
                html += '<li class="list-group-item"><strong>' + result.phrase + '</strong><br>' + result.reason + '</li>';
            }
        }
        html += '</ul>';
